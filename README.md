...
| `instance-initiated-shutdown-behavior`                                                                                                                                       | Optional. Used only with the `start` mode. | Specifies how the instance should behave when shut down. Default: `stop` just stops the instance but it can be restarted, `terminate` completely terminates the instance. |
<br><br> |
